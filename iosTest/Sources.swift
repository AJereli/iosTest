        ...
        return Promise{ fulfill, reject in
            ...
            for item in unParsedItems{
                ...
                if !items.contains(where: {
                 ...
                 let item = Item(id: id, title: unParsedItem["title"]!, imageUrl: URL(string: unParsedItem["imageUrl"]!)!, description: unParsedItem["description"]!, newsUrl: unParsedItem["newsUrl"]!)!
                    items.append(item)
                    cnt += 1
                }
...
class Sources {
    ...
    static func getInstance () -> Sources{
        if (sources == nil){
            sources = Sources()
        }
        return sources
    ...
    func loadItems (limitForSource:Int) -> [Item]{
        ...
        for source in favoritsSources{
            let countOldItems = source.items.count
            let newItems = source.loadItemsFromSource(limit: limitForSource).value!
    ...
    private func createMockItems (){
        let stringJson:String = """
        {
            "https://habrahabr.ru": [{
                    ...
                    "imageUrl": "https://habrastorage.org/getpro/habr/post_images/4a2/de3/232/4a2de32322fd0a1d353ffae701f5ec702.jpg",
